    ...
    public class EntityConverter : IEntityConverter
    {
        ...
        public bool IsGeometricEntity(Entity entity)
        {
            return entity is Curve ||
                   ...
                   entity is MText ||
                   entity is DBText ||
                   entity is Dimension ||  // 新增：尺寸標註
                   entity is Leader ||     // 新增：引線
                   entity is MLeader ||    // 新增：多重引線
                   entity is Table ||      // 新增：表格
                   entity is Polyline ||
                   ...
                   entity is Circle ||
                   entity is Ellipse ||
                   entity is DBPoint ||    // 修正：使用DBPoint而不是Point
                   entity is Spline ||
                   entity is Ray ||
                   entity is Xline ||
                   entity is Shape;
            ...
            try
            {
                ...
                if (layerTable != null && layerTable.Has(sourceLayerName))
                {
                    ...
                    try
                    {
                        ...
                        {
                            result.ConvertedCount++;
                            result.Errors.Add($"已處理鎖定圖層上的{GetEntityTypeName(entity)}: {entity.GetType().Name}");
                        }
                        else
                        {
                            result.ErrorCount++;
                            result.Errors.Add($"無法轉換鎖定圖層上的{GetEntityTypeName(entity)}");
                        }
                    }
                    finally
                    {
                ...
                {
                    result.ErrorCount++;
                    result.Errors.Add($"找不到源圖層: {sourceLayerName}");
                }
            }
            catch (System.Exception ex)
            {
                result.ErrorCount++;
                result.Errors.Add($"處理鎖定圖層上的標註失敗: {ex.Message}");
            }
        ...
        private string GetEntityTypeName(Entity entity)
        {
            return entity switch
            {
                ...
                Leader => "引線",
                MLeader => "多重引線",
                BlockReference blockRef when IsDynamicBlock(blockRef) => "動態圖塊",
                BlockReference => "圖塊",
                _ => "物件"
            };
        }
        ...
        private bool TryConvertEntityLayer(Transaction tr, Entity entity, string targetLayer, ConversionOptions options)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"轉換實體圖層失敗: {ex.Message}");
                return false;
            }
        }
    }
